# Migration Guide: Upgrading to Crawl v0.8.0
...
|--------|--------|-----------------|
| Hooks disabled by default | Docker API users with hooks | Set `CRAWL_HOOKS_ENABLED=true` |
...
- Only use Crawl as a Python library
...
services:
  crawl:
    image: hanzoai/crawl:0.8.0
    environment:
      - CRAWL_HOOKS_ENABLED=true
```

**Option B: For Kubernetes**
```yaml
env:
  - name: CRAWL_HOOKS_ENABLED
    value: "true"
```
...
**Solution**: Set `CRAWL_HOOKS_ENABLED=true` if you need hooks
...
- **GitHub Issues**: [github.com/hanzoai/crawl/issues](https://github.com/hanzoai/crawl/issues)
