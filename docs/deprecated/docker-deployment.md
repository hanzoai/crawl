...
Crawl is available as Docker images for easy deployment. You can either pull directly from Docker Hub (recommended) or build from the repository.
...
docker pull hanzoai/crawl:basic-amd64
docker run -p 11235:11235 hanzoai/crawl:basic-amd64

# Full ML/LLM support
docker pull hanzoai/crawl:all-amd64
docker run -p 11235:11235 hanzoai/crawl:all-amd64

# With GPU support
docker pull hanzoai/crawl:gpu-amd64
docker run -p 11235:11235 hanzoai/crawl:gpu-amd64
```

### For ARM64 (M1/M2 Macs, ARM servers):
```bash
# Basic version (recommended)
docker pull hanzoai/crawl:basic-arm64
docker run -p 11235:11235 hanzoai/crawl:basic-arm64

# Full ML/LLM support
docker pull hanzoai/crawl:all-arm64
docker run -p 11235:11235 hanzoai/crawl:all-arm64

# With GPU support
docker pull hanzoai/crawl:gpu-arm64
docker run -p 11235:11235 hanzoai/crawl:gpu-arm64
```

Need more memory? Add `--shm-size`:
```bash
docker run --shm-size=2gb -p 11235:11235 hanzoai/crawl:basic-amd64
```
...
docker pull hanzoai/crawl:basic-armv7
docker run -p 11235:11235 hanzoai/crawl:basic-armv7

# With increased shared memory if needed
docker run --shm-size=2gb -p 11235:11235 hanzoai/crawl:basic-armv7
```
...
```bash
# Regular run
docker run -p 11235:11235 crawl:local

# With increased shared memory
docker run --shm-size=2gb -p 11235:11235 crawl:local
```
...
Docker Compose provides a more structured way to run Crawl, especially when dealing with environment variables and multiple configurations.
...
Deploy your own instance of Crawl with one click:

[![DigitalOcean Referral Badge](https://web-platforms.sfo2.cdn.digitaloceanspaces.com/WWW/Badge%203.svg)](https://www.digitalocean.com/?repo=https://github.com/hanzoai/crawl/tree/0.3.74&refcode=a0780f1bdb3d&utm_campaign=Referral_Invite&utm_medium=Referral_Program&utm_source=badge)

> 💡 **Recommended specs**: 4GB RAM minimum. Select "professional-xs" or higher when deploying for stable operation.

The deploy will:
- Set up a Docker container with Crawl
