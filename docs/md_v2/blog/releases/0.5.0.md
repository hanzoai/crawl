# Crawl v0.5.0 Release Notes

**Release Theme: Power, Flexibility, and Scalability**

Crawl v0.5.0 is a major release focused on significantly enhancing the
...
Crawl now supports deep crawling, allowing you to explore websites beyond the
...
```python
import time
from crawl import AsyncWebCrawler, CrawlerRunConfig, BFSDeepCrawlStrategy
from crawl.content_scraping_strategy import LXMLWebScrapingStrategy
from crawl.deep_crawling import DomainFilter, ContentTypeFilter, FilterChain, URLPatternFilter, KeywordRelevanceScorer, BestFirstCrawlingStrategy
import asyncio
...
```python
from crawl import AsyncWebCrawler, CrawlerRunConfig, MemoryAdaptiveDispatcher
import asyncio
...
async def batch_mode():
    async with AsyncWebCrawler() as crawler:
        results = await crawler.arun_many(
            urls=["https://docs.hanzo.ai", "https://github.com/hanzoai/crawl"],
...
Crawl now offers two crawling strategies:
...
```python
from crawl import AsyncWebCrawler, CrawlerRunConfig, HTTPCrawlerConfig
...
Crawl can now be easily deployed as a Docker container, providing a
...
A new CLI (`crwl`) provides convenient access to Crawl's functionality from
the terminal.
...
- **Added: PDF processing capabilities.** Crawl can now extract text, images,
...
- **Added:** Modified the license to Apache 2.0 _with a required attribution
  ...
  attribute the Crawl project when using, distributing, or creating
  derivative works.
