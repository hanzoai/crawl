...
# C4ai version
ARG CRAWL_VER=0.9.2
ENV CRAWL_VERSION=$CRAWL_VER
LABEL crawl.version=$CRAWL_VER

# Set build arguments
ARG APP_HOME=/app
ARG GITHUB_REPO=https://github.com/hanzoai/crawl.git
...
LABEL description="🔥🕷️ Crawl: Open-source LLM Friendly Web Crawler & scraper"
LABEL version="1.0"
...
if [ "$USE_LOCAL" = "true" ]; then\n\
    ...
    for i in {1..3}; do \n\
        git clone --branch ${GITHUB_BRANCH} ${GITHUB_REPO} /tmp/crawl && break || \n\
        { echo "Attempt $i/3 failed! Taking a short break... ☕"; sleep 5; }; \n\
    done\n\
    pip install --no-cache-dir /tmp/crawl\n\
...
RUN pip install --no-cache-dir --upgrade pip && \
    /tmp/install.sh && \
    python -c "import crawl; print('✅ crawl is ready to rock!')" && \
...
RUN mkdir -p /var/lib/crawl/outputs \
    && chown -R appuser:appuser /var/lib/crawl \
    && chmod 700 /var/lib/crawl/outputs
