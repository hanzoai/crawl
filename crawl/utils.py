...
class VersionManager:
    def __init__(self):
        self.home_dir = Path(os.getenv("CRAWL4_AI_BASE_DIRECTORY", Path.home())) / ".crawl"
...
class RobotsParser:
    ...
    def __init__(self, cache_dir=None, cache_ttl=None):
        self.cache_dir = cache_dir or os.path.join(get_home_folder(), ".crawl", "robots")
...
def get_home_folder():
    """
    Get or create the home folder for Crawl configuration and cache.

    How it works:
    1. Uses environment variables or defaults to the user's home directory.
    2. Creates `.crawl` and its subdirectories (`cache`, `models`) if they don't exist.
    3. Returns the path to the home folder.

    Returns:
        str: The path to the Crawl home folder.
    """
...
async def get_chromium_path(browser_type) -> str:
    ...
    async with async_playwright() as p:
        ...
        # Save the path int the crawl home folder
        home_folder = get_home_folder()
...
def configure_windows_event_loop():
    ...
    Example:
        ```python
        from crawl.async_configs import configure_windows_event_loop
...
def setup_colab_environment():
    ...
    print("🚀 Setting up Crawl environment in Google Colab...")
    ...
    # Default: use sentence-transformers
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install 'crawl[transformer]' or pip install sentence-transformers"
            )
