...
import argparse
from crawl.config import MODEL_REPO_BRANCH
...
def get_home_folder():
    home_folder = os.path.join(
        os.getenv("CRAWL4_AI_BASE_DIRECTORY", Path.home()), ".crawl"
    )
...
def load_spacy_model():
    ...
    if not (model_folder.exists() and any(model_folder.iterdir())):
        repo_url = "https://github.com/hanzoai/crawl.git"
        branch = MODEL_REPO_BRANCH
        repo_folder = Path(home_folder) / "crawl"
...
def download_all_models(remove_existing=False):
    """Download all models required for Crawl."""
    if remove_existing:
...
def main():
    print("[LOG] Welcome to the Crawl Model Downloader!")
    print("[LOG] This script will download all the models required for Crawl.")
    parser = argparse.ArgumentParser(description="Crawl Model Downloader")
    parser.add_argument(
        "--remove-existing",
        action="store_true",
