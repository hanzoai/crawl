"""
Crawl Cloud CLI - Commands for interacting with Crawl Cloud service.
...
from crawl.browser_profiler import ShrinkLevel, _format_size

console = Console()

# Default cloud API URL
DEFAULT_CLOUD_API_URL = "https://crawl.hanzo.ai"


def get_global_config() -> dict:
    """Load global config from ~/.crawl/global.yml"""
...
def save_global_config(config: dict):
    """Save global config to ~/.crawl/global.yml"""
...
def require_auth() -> tuple[str, str]:
    ...
    if not api_key:
        console.print("[red]Not authenticated with Crawl Cloud.[/red]")
...
@click.group("cloud")
def cloud_cmd():
    """Crawl Cloud commands - manage cloud profiles and authentication.

    Use browser profiles for authenticated crawling in the cloud.

    Getting started:
      1. Get an API key at https://crawl.hanzo.ai/dashboard
...
def auth_cmd(api_key: str, api_url: str, logout: bool, status: bool):
    """Authenticate with Crawl Cloud.

    Your API key is saved locally in ~/.crawl/global.yml

    To get an API key:
      1. Go to https://crawl.hanzo.ai/dashboard
    ...
    if status:
        ...
        if current_key:
            ...
            console.print(Panel(
                ...
                "Run [cyan]crwl cloud auth[/cyan] to authenticate.\n\n"
                "Get your API key at:\n"
                "[blue]https://crawl.hanzo.ai/dashboard[/blue]",
                title="Cloud Auth Status",
                border_style="yellow"
            ))
        return
    ...
    if not api_key:
        console.print(Panel(
            "[cyan]Crawl Cloud Authentication[/cyan]\n\n"
            "To get your API key:\n"
            "  1. Go to [blue]https://crawl.hanzo.ai/dashboard[/blue]\n"
            "  2. Sign in or create an account\n"
            "  3. Navigate to API Keys section\n"
    ...
    console.print(f"Credentials saved to [cyan]~/.crawl/global.yml[/cyan]")
...
def profiles_cmd():
    ...
    Upload local browser profiles to Crawl Cloud for authenticated crawling.
...
def upload_cmd(profile_name: str, name: str, level: str, no_shrink: bool):
    """Upload a browser profile to Crawl Cloud.
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="crawl_upload_"))
...
def list_cmd():
    ...
    Shows all profiles uploaded to your Crawl Cloud account.
    """
    api_key, api_url = require_auth()
