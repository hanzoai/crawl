...
def setup_home_directory():
    """Set up the .crawl folder structure in the user's home directory."""
    base_dir = os.getenv("CRAWL4_AI_BASE_DIRECTORY")
    crawl_folder = Path(base_dir) if base_dir else Path.home()
    crawl_config = crawl_folder / "global.yml"
    crawl_folder = crawl_folder / ".crawl"
    cache_folder = crawl_folder / "cache"
    content_folders = [
        "html_content",
        "cleaned_html",
        "markdown_content",
    ...
    crawl_folder.mkdir(exist_ok=True)
    cache_folder.mkdir(exist_ok=True)
    for folder in content_folders:
        (crawl_folder / folder).mkdir(exist_ok=True)
...
def post_install():
    ...
    Checks CRAWL_MODE environment variable. If set to 'api',
    ...
    run_mode = os.getenv('CRAWL_MODE')
    if run_mode == 'api':
        logger.warning(
            "CRAWL_MODE=api detected. Skipping Playwright browser installation.",
            tag="SETUP"
        )
    else:
...
def setup_builtin_browser():
    """Set up a builtin browser for use with Crawl"""
    try:
        ...
        logger.warning("You can manually set up a builtin browser using 'crawl-doctor builtin-browser-start'")
...
async def run_doctor():
    """Test if Crawl is working properly"""
    logger.info("Running Crawl health check...", tag="INIT")
    try:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            result = await crawler.arun(url="https://crawl.hanzo.ai", config=run_config)
